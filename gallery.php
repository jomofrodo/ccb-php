...
if(0) print_r($args);
extract($args);
if(! $gallery_pid) $gallery_pid = $cat_pid;
$cat_table = todosGetCatTableName($gallery_pid);
$rec_num = $_POST['rec_num'];
if(! $rec_num){ //find position in gallery based on rec_pid
    $rec_num = getRecPosition($rec_pid,$err);
}
...
<? if($flgEnd) print(" disabled=true ");?> name="next photo" style="padding-right:20px;padding-bottom:10px;
...
function getRecPosition($rec_pid,&$err){
    // Find the postiion of a record in the gallery
    GLOBAL $cat_table;
    $recCt = 0;
    $sql = todosGenCatSQL($cat_table);
    $rs = todosExecSQL($sql);
    while(! $rs->EOF){
        $img_pid = $rs->fields[FLD_PID];
        if($img_pid == $rec_pid)return($recCt);
        $recCt++;
        $rs->MoveNext();
    }
    //If we get here, record was not found
    // return 0
    $err = "Record not found in this gallery";
    return(0);
}
