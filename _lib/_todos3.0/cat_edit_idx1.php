...
<?php
  ...
  $page_short_title="Add New Category";
	...
	## Set Page Variables
      ...
      $txtLinkRecursive    = "Link all pages underneath this directory?";<p>     //cat Name
     ...
     //cat PID<p>    // check if an index file exists
    //$idxpath = convPID2Path($idx_pid);
     $feIdx = file_exists($idxpath);<p>     //Page type
     $td_class = EO_CLASS_CATEGORY;
	 if(0) var_dump($args);<p>	switch($pAction){<p>        case "SaveParams":
...
<?
   ...
   default:
              ...
              $cat_pid = $new_cat_pid;
               ...
               todosEditCHash($td,$td_class,$cols,$flgClassOnly,$paramList);<p>    	echo(" </td></tr>\n");
	echo(" <tr><td>&nbsp;\n");
