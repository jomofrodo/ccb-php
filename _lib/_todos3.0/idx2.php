...
<?
  ...
  //      file -- ??<p>  $nav_global="admin";
  $nav_section="";
  $page_short_title="Todos Glimpse";<p>   session_register("GlimpseDir");
   //$excluded = "^_|^\.[A-Za-z]|CVS";
   $excluded = "^\.[A-Za-z]|CVS";<p>?>
<?
#####################################################################################
  //This is GLIMPSE
...
##	Thanks Derek!
  ...
  #$basedir= "/www/servers/Dwyer/d3dev/upload";
   ...
   if(0) print_r($args);
    ...
    $files = getDirFiles($dir,'*',$strFilter,$excluded,0);<p>    //Todos
    $rec_pid = $rec_pid;
    $cat_pid = $cat_pid;
    $dir = $dir;
...
?>
<?php
// Page Specific Utility Functions<p>function checkMembers($rec_pid,$files,$dir){
   //Debugging routine
   ...
   // 0 = non-member
    ...
    return(0);
}<p>function getUnlinkFiles($files,$link_files,$dir){
	...
	if($debug) print_r($unlink_files);<p>	return($unlink_files);
}<p>function getUnlinkSubcats($files,$link_subcats,$dir){
	// Get the list of files in this directory
	// not linked to the parent
	// returns an array
	$i=0;
	...
	// hmmm , that was easy
	return($unlink_cats);
}<p>function reloadnow($addons) {
    global $URI;
    //global $addons;
    header("Status: 302 Moved");
    header("Location: $URI" . $addons);
    exit(); }
?><p><form name=frmLink action="<?=$URI?>" ENCTYPE="multipart/form-data" method=POST>
...
<? //Table Header
  ...
  // start HTML
?><p><div align=center>
<TABLE BORDER="1"><p><tr><td colspan=5><p>    <table border=0><tr><td >
...
<input type=button value="Link All" onClick="this.form.gAction.value='LinkAll';this.form.submit()"><p><input type=button value="UnLink All" onClick="this.form.gAction.value='UnlinkAll';this.form.submit()"><p><input type=button name="btnLU" value="Link/Unlink"  onClick="this.form.gAction.value='Link';this.form.submit()"><p></TABLE>
<BR><p><INPUT NAME="userfile" TYPE="file"><p>    <INPUT TYPE="Button" NAME="upload" VALUE="Upload" onClick="this.form.gAction.value='Upload';this.form.submit()"><BR><p><INPUT TYPE="TEXT" NAME="touchfile" ><p>    <INPUT TYPE="Button" NAME="touch"  VALUE="Touch" onClick="this.form.gAction.value='Touch';this.form.submit()">
<?         echo " l/u<INPUT name=link_touch type=checkbox value=1";
           echo " checked ><BR>";
           ?><p><INPUT TYPE="TEXT" NAME="mkdirfile"><p>    <INPUT TYPE="Button" NAME="mkdir"  VALUE="Mkdir" onClick="this.form.gAction.value='MkDir';this.form.submit()"><BR><p></FORM><p></table><p></div><p><p></BODY><p><?php include_once($_SERVER[DOCUMENT_ROOT] . "/_include/cf.php"); ?><p> <script language="javascript"><p>	if(document.forms['frmRecEdit']){<p>		//$idx = 'code';<p>        $idx ='<?=$focusElement?>';<p>        if(! $idx) $idx = 1;<p>		$form = document.forms['frmRecEdit'];<p>		$length = $form.elements.length;<p>		$elem = $form.elements[$idx];<p>		//alert($elem.type);<p>		if(($elem.type == 'hidden')&&($idx < $length)){<p>			$idx++;<p>			$elem = $form.elements[$idx];<p>		}<p>		//alert($elem.name);<p>		if($elem.type == 'text'){<p>			$elem.focus();<p>			$elem.select();<p>		}<p>	}<p></script><p></HTML>
