...
#################################################################*/<p>//print("Hello World"); <p>include_once(dirname(__FILE__)."/todosInclude.php");
include_once(dirname(__FILE__)."/lib_todos.php");<p>
if(!@$gDBtd->PConnect($DB_LOCATION, $DB_ACCOUNT, $DB_PASSWORD, $DB_DATABASE)) { lib_login_db_failure(); }
/*============================ GLOBAL-STUFF ===========================*/
/*============================ ^^^^^^^^^^^^ ==========================*/<p>GLOBAL	  $nav_global; 		// global navigational section name
...
GLOBAL	  $cat_pid;		// category pid<p><p>//################################################################
...
//return the list<p>  GLOBAL $gDBtd;<p>//Default td_type is the DOC type
if ($td_type=='x'){$td_type=IDX_3;}
if ($td_class=='x'){$td_class=EO_CLASS_SUBDOC;}
if ($page_id==""){$page_id = "/";}<p>	### Using the idx3_class tables !!!
	$tbl = '';
	...
	if(! $tbl) $tbl = 'tblTodos';<p>//############
// DEBUG
//$page_id = "page1";
//############<p>  $db = $gDBtd;<p>//       $sql_a =<<<SQL
//		SELECT  *
//		from 	tbl_users;
//SQL;
       $sql_a =<<<SQL
...
GLOBAL	  $navPage;
GLOBAL	  $navSide;
GLOBAL	  $cat_pid;<p>   if(0) print("setNavVars(0): $flgSide,$flgSide2,$flgSide3,$global_id,$page_id,$cat_pid<br>");<p>  //$url = sprintf("%s%s%s","http://",$HTTP_HOST,$REQUEST_URI);
  ...
  $str_Application_Class = 'application';	// <MAGIC STRING><p>  //print_r ("PageID: $page_id <BR>");<p>  $path = pathinfo($url["path"]);
  $dir 	= $path["dirname"];
  $file = $path["basename"];
  $ext 	= $path["extension"];
...
if(0){
  ...
  print_r ("Ext: $ext <BR>");
}<p>  $char0 = substr($page_id,0,1);
  if ($char0 != "/"){$page_id = ("/" . $page_id);} <p>  ($nav_global) ? $x=1		:  	setNavGlobal($dir);
...
//  ($page_apps) ? $x=1		: 	setPageDocs($page_id,$page_apps,'idx3',EO_CLASS_APPS);
  ...
  ($nav_page_id) ? $x=1		:	$nav_page_id = $page_id;<p>  $global_id = "/" . $nav_global . "/idx";
  if(0) print("setNavVars(1): $flgSide,$global_id,$page_id,$cat_pid<br>");
  $cat_pid = getCatPID();
   if(0) print("cat_pid: $cat_pid<br>");<p>  ## Set Nav Bars
	...
	//$navPage = getNavPage($page_id,$cat_pid,0,$flgSide,$flgSide3);
     ...
     ## Side Nav
	if($flgSide){ $navSide = getNavSide($global_id,$cat_pid,0);}<p>     if(0)	print("setNavVars(2): $flgSide,$global_id,$page_id,$cat_pid<br>");
		<p>if(0) print("setNavVars: $page_id,$nav_global,$nav_section,$page_title,$page_short_title,$cat_pid<br>");
...
// based on the URL and passed in variables
 ...
 if(0) var_dump("$dir<br>");
  ...
  $dir =  $dir[1]; 	//get the root level directory<p>   $nav_global = $dir;
//  switch($dir) {
...
//
  GLOBAL $nav_global; 
  GLOBAL $page_title; <p>	$page_title = todosGetTitle($page_id);
	if(0) print("setPageTitle: todosTitle: $page_title<br>");
	//var_dump($page_id);
	//var_dump($page_title);exit;
	if($page_title){return($page_title);}<p>	// If no idx0 tdTitle, calculate from file name<p>  $page_title = explode('.',$file);  
...
//
  ...
  //$page_short_title = ucwords($t);<p>  $st = $t;
...
function gNavLink($nav_section,$menu='',$x='',$y='',$nav_name='',$nav_link=''){
  ...
  //Switch the $nav_link based on the global tag
	//print("\n" .$nav_global."\n");
	//print("<BR>");
	//exit;<p>GLOBAL $nav_global;<p>  if(! $nav_link){
   switch($nav_section) {
  ...
  $html .= " class=topnav>" . $nav_name . "</a>&nbsp;&nbsp;\n";<p>  ##if(0) print("gNavLink: $nav_section,$nav_link,$nav_name,$nav_global<br>");
  print($html);
}
//################################################################
...
// Print a side bar navigation select
  ...
  //Check for a session variable
	...
	}<p>   $idx_file = INDEX_FILE;<p>   preg_match('/^(.*)\//',$page_id,$navg);
   $nav_dir = $navg[1];
   ...
   $td3 = getTodos($rs);    //Array of IDX3 linked docs
	##if(0) printTodos($td3);<p>   $cat_title = todosGetVal($cat_pid,IDX_0,'category',TD_PNAME_TITLE,TD_VALTYPE_FLD_TDTITLE);
   ...
   if(! $y){
	$_SESSION[$navSide] = '';
	return($_SESSION[$navSide]);
   }<p>    // Lots of subnavs . . .<p>	  $select_name = 'cat_pid';
  ...
  ## DEBUG 
	...
	if(0) print("HTML: $html<br>");
   	##if(0)printTodos($td3);
	##if(0) exit;
  ## DEBUG <p>	##if(0) var_dump($html);
	if(0) print("flgSideMenu: $flgSide,$flgSideMenu,$y<br>");<p>	if($flgPrint) print($html);
	$_SESSION[$navSide] = $html;
  return($html);
}
//################################################################
function getNavSection($page_id,$cat_pid,$flgPrint=1,$flgSide=0){
// Print the correct settion nav bar for the current page<p>   if(0) print("getNavSection: $page_id,$cat_pid,$flgPrint,$flgSide<br>");
   $ltgrnbox = "<img src='". IMG_BULLET1 ."'>";
   $navSection = 'nav_' . $page_id;
   $flgSideName = $navSection . 'flgSide';
	<p>   $idx_file = INDEX_FILE;<p>   $navg = explode('/',$page_id);
   $nav_dir = $navg[1];
   ...
   $td3 = getTodos($rs);    //Array of IDX3 linked docs
	##if(0) printTodos($td3);<p>   $cat_title = todosGetVal($cat_pid,IDX_0,'category',TD_PNAME_TITLE,TD_VALTYPE_FLD_TDTITLE);
   if(0) print("navSection catPID/page_id: $cat_pid,$page_id<br>");<p>   if($flgSelf){
   ...
   $nav_bar = $td3;
   $y = count($nav_bar);<p>
   $array_html = array();<p>	## if(($y < 7) || ($flgSide == -1)){
	if(! $flgSide){
 	   ...
 	   // Lots of subnavs . . .
	  $flgSideMenu = 1;<p>	  $select_name = 'cat_pid';
	  $current_val = preg_replace('/index.php/','',$page_id);	
	  $flgAll	= 0;
	  $valAll	= $nav_dir;
	  $flgSelect 	= 0;
...
//	return html value<p>	if(0) print("getNavPage: $rec_pid,$flgPrint,$flgSide,$flgSideMenu<br>");<p>        if(0) print("navPage: $rec_pid<br>");
	$rec_pid = convURL2PID($rec_pid);
        $navPage = '';
	##<WEIRD>
	$cat_pid = $_GET['cat_pid'];
	if(! $cat_pid) $cat_pid = getCatPID($rec_pid);
	...
	if(0) print("navPage: $navPage<br>");<p>	if($flgPrint) print($navPage);
	return($navPage);
}<p>//################################################################
...
// Return the block<p>   GLOBAL $nav_global;<p>   $td_class = EO_CLASS_SUBNAV;
...
function arrowIf($this_global){
  ...
  if(preg_match($match,strtoupper($nav_global))){
	$imgsrc = TODOS_ROOT . "/images/goldarrow.gif";
	##if(0) print("arrow: $imgsrc<br>");
      return($imgsrc );
  }
  else {
     return(TODOS_ROOT . "/images/spacer.gif width=15");
  }
}<p>//################################################################
function gNavName($this_global){
  ...
  GLOBAL $nav_global;
  $nav_name;		//Display name for global nav section<p>  //print $this_global; exit;
