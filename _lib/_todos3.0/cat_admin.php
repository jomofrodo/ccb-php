...
<?php<p>    // first we have to do our includes
	include_once(dirname(__FILE__)."/lib_todos.php");
	include_once(dirname(__FILE__)."/_inc_hdr.php");<p>  // Page to list a category summary
  // AS of this entry, todos is in mySQL database todosDI on lucy<p>  $nav_global="admin";
  $nav_section="";
  $page_short_title="Todos Admin";<p>  $dbgsessid="";  //DEBUG
	### Process POST or GET args<p>		if(0) var_dump($args);
		if(0) var_dump($_REQUEST);
		if(0) var_dump($_SERVER);
		fixFormVars($args);
	### Set some argument defaults
		if(! $args['rspage']) $args['rspage']=1;
		if(! $args['pAction']) $args['pAction'] = 'Category';<p>		## For now, always in admin mode
		$modeAdmin = 1;
		if($modeAdmin) $args['flgEdit'] = 1;<p>	## Set Page Variables
		...
		switch($pAction){
			...
			case "Link"	:
					$url = TODOS_ROOT . PAGE_IDX2_EDIT .
						"?cat_pid=$cat_pid&td_class=$td_class";
					  header("Location:$url");
					  break;
			...
			case "ReCalc"	:
					...
					break;
			case "EditCat"	:
					$url = TODOS_ROOT . PAGE_REC_EDIT .  "?cat_pid=$cat_pid";
					ob_end_clean();
					header("Location:$url");
					break;
			case "Commit"	:
...
<form name=frmSrchMain method=get action="">
  ...
  <table>
    ...
    <tr>
     ...
     <tr> 
       <td colspan=4 align=center> <p>	<input type=submit value="ReCalc Category" onclick="this.form.pAction.value='ReCalc';">
...
<form>
<?<p>		if(0) print("pAction: $pAction<br>");
		switch($pAction){
			case "GoCategory" : 
					$args['rspage']=1;
					if(0) var_dump($args);
