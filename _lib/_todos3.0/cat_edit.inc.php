...
<?
    // first we have to do our includes
     ...
     include_once(dirname(__FILE__)."/_inc_hdr.php");<p>  $nav_global="admin";
  $nav_section="";
  $page_short_title="Todos Admin";
  global $page_id;
 <p>	$MODES = array('IDX1'=>'Properties',
			'IDX3'=>'Relations',
			'CAT'=>'Sub-Categories',	
			'NAV'=>'Sub-Nav',
			'ADM'=>'Members');<p>
	...
	if(! $pAction) $pAction = 'editHash';
		...
		$cat_pid='';<p>		#$td_class = EO_CLASS_CATEGORY;
		$class = EO_CLASS_CATEGORY;
		$sql_class = "AND tdType='idx0' AND tdClass='" . EO_CLASS_CATEGORY . "'";<p>		$args = $_POST;	
		if(! $args) $args = $_GET;
		...
		switch($pAction){
			...
			default		: 
					...
					break;<p>		}
		$td = todosGetClassHash($cat_pid,$class,'',$debug); <p>	###### DEBUG ############
	$args['debug'] = 0;
...
<html>
	...
	<body>
		<p><form name=frmTodosAdmin method=post enctype="multipart/form-data">
  ...
  <table border=0>
	...
	<td><?=$cat_pid?>
      <tr>
	<th colspan=4><p>	<? foreach($MODES as $m=>$title){
