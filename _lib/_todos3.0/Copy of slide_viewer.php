...
<? include_once ('./lib_todos.php');  ?><p><?php
// Todos Slide Viewer
// Jomo 01/05
...
//      < >         // next/previous buttons<p>if(0) print_r($args);
if(! $cat_pid) $cat_pid = $_GET['cat_pid'];
$rec_num = $_POST['rec_num'];<p>switch($pAction){
    case 'goForward':
            $rec_num++;
            break;
    case 'goBack':
...
if($rec_num == ($num_recs-1)) $flgEnd = 1;<p>function getNumImages($cat_pid,&$err){
